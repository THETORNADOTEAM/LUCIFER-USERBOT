...
@LUCIFERbot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@LUCIFERbot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {mafia}\n\n⚡ **[LEGENDARY AF LUCIFEROT](t.me/LUCIFER_SUPPORT_GROUP)** ⚡"
...
@LUCIFERbot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@LUCIFERbot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
async def uninstall(kaal0408):
    if kaal0408.fwd_from:
        return
    shortname = kaal0408p.pattern_match["shortname"]
